...
if (isset($_POST['hotel_name_submit'])) {
    $old = $_POST;
    $result = hotel_name();
...
if (isset($_POST['hotel_name_delete'])) {
    $result = hotel_name_delete();
    if ($result['status'] == 'error') {
    ...
    <div class="w-4/5 overflow-auto p-4">
        <div class="container mx-auto pt-24">
            ...
            <form method="post" class="ps-12 w-full" enctype="multipart/form-data">
                ...
                <div class="mt-6 gap-x-4 flex justify-start items-center">
                        <div class="flex justify-start items-center outline outline-1 outline-black rounded w-full">
                            <label for="hotel_name" class="text-black text-xl w-40 ps-4 font-serif">Hotel Name: </label>
                            <input type="text" name="hotel_name" value="<?php echo $old['hotel_name'] ?? ''; ?>" placeholder="Enter Hotel Name" 
                            class="py-2 bg-transparent w-full px-4 focus:outline-none">
                        </div>
                    <button type="submit" name="hotel_name_submit" class="bg-cyan-950 hover:bg-slate-800 text-white transition duration-700 ease-in-out font-serif text-lg font-semibold px-8 py-1.5 rounded outline outline-1 outline-black">Submit</button>
                </div>
                <h5 class="text-black font-mono font-xl my-2 text-center"><?php echo $error['hotel_name'] ?? ''; ?></h5>
            </form>
                ...
                <table class="table-fixed w-full mt-8 rounded-lg border-collapse overflow-hidden ">
                    <thead class="bg-cyan-950 text-white">
                        <tr class="text-center">
                            ...
                            <th class="py-2 px-4 w-full border border-black">All Hotel Names</th>
                            <th class="py-2 px-4 w-60 border border-black">Status</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                        if (mysqli_num_rows($hotel_name_view) > 0) {
                            $i = $starting_limit + 1;
                            while ($row = mysqli_fetch_assoc($hotel_name_view)) {
                                echo '<tr class="text-center bg-white rounded">';
                                    ...
                                    echo '<td class="py-2 px-4 w-full font-serif font-semibold text-gray-800 border border-black rounded">' . $row["hotel_name"] . '</td>';
                                    echo '<td class="py-2 px-4 w-60 font-serif font-semibold text-gray-800 border border-black rounded">';
                                        echo '<div class="flex justify-center items-center">';
                                            echo '<form method="post" onsubmit="return confirm(\'Do you want to delete?\')">
                                                    ...
                                                    <button class="fs-4 mx-4 bg-red-700 py-1 px-2 rounded" name="hotel_name_delete" type="submit">
            ...
            <div class="mt-8 flex justify-center">
                <?php if ($page > 1): ?>
                    <a href="hotel.php?page=<?php echo $page - 1; ?>" class="px-8  py-2 mx-1 text-white rounded bg-cyan-950 transition duration-700 ease-in-out  hover:bg-transparent hover:outline-1 hover:outline hover:outline-black hover:text-black">Previous</a>
                <?php endif; ?>

                <?php for ($p = 1; $p <= $total_pages; $p++): ?>
                    <a href="hotel.php?page=<?php echo $p; ?>" class="px-8  py-2 mx-1 border border-<?php if ($p == $page){echo 'white';}else{echo 'black';}  ?> rounded <?php if ($p == $page) echo 'bg-cyan-950  text-white'; ?>">
                        <?php echo $p; ?>
                    </a>
                <?php endfor; ?>

                <?php if ($page < $total_pages): ?>
                    <a href="hotel.php?page=<?php echo $page + 1; ?>" class="px-8  py-2 mx-1 text-white rounded bg-cyan-950 transition duration-700 ease-in-out  hover:bg-transparent hover:outline-1 hover:outline hover:outline-black hover:text-black">Next</a>
                <?php endif; ?>
            </div>
        </div>
    </div>
