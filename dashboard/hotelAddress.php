...
if (isset($_POST['addressSubmit'])) {
    ...
    if ($result['status'] == 'error') {
        ...
        header('Refresh: 1; URL=hotelAddress.php');
    }
}

if (isset($_POST['hotelAddressDelete'])) {
    $result = hotelAddressDelete();
    if ($result['status'] == 'error') {
...
$total_results_query = mysqli_query(db_connect(), "SELECT COUNT(*) as total FROM hotelAddress");
...
$hotelAddressView = hotelAddressView($starting_limit, $results_per_page);
?>
<div class="flex">  
    ...
    <div class="w-4/5 overflow-auto p-4">
        <div class="container mx-auto pt-24">
            <div class="flex justify-between items-center mb-4">
                <h1 class="text-3xl font-bold font-serif capitalize ps-12 ">Hotels</h1>
            ...
            <form method="post" class="ps-12 w-full" enctype="multipart/form-data">
                ...
                <div class="mt-6 gap-x-4 flex justify-start items-center">
                        <div class="flex justify-start items-center outline outline-1 outline-black rounded w-full">
                            <label for="address" class="text-black text-xl w-60 ps-4 font-serif">Hotel Address: </label>
                            <input type="text" name="address" value="<?php echo $old['address'] ?? ''; ?>" placeholder="Hotel Address" 
                            class="py-2 bg-transparent w-full px-4 focus:outline-none">
                        </div>
                    <button type="submit" name="addressSubmit" class="bg-cyan-950 hover:bg-slate-800 text-white transition duration-700 ease-in-out font-serif text-lg font-semibold px-8 py-1.5 rounded outline outline-1 outline-black">Submit</button>
                </div>
                <h5 class="text-black font-mono font-xl my-2 text-center"><?php echo $error['address'] ?? ''; ?></h5>
            </form>
                ...
                <table class="table-fixed w-full mt-8 rounded-lg border-collapse overflow-hidden ">
                    <thead class="bg-cyan-950 text-white">
                        <tr class="text-center">
                            ...
                            <th class="py-2 px-4 w-full border border-black">Hotel Address</th>
                            <th class="py-2 px-4 w-60 border border-black">Status</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                        if (mysqli_num_rows($hotelAddressView) > 0) {
                            $i = $starting_limit + 1;
                            while ($row = mysqli_fetch_assoc($hotelAddressView)) {
                                echo '<tr class="text-center bg-white rounded">';
                                    ...
                                    echo '<td class="py-2 px-4 w-full font-serif font-semibold text-gray-800 border border-black rounded">' . $row["address"] . '</td>';
                                    echo '<td class="py-2 px-4 w-60 font-serif font-semibold text-gray-800 border border-black rounded">';
                                        echo '<div class="flex justify-center items-center">';
                                            echo '<form method="post" onsubmit="return confirm(\'Do you want to delete?\')">
                                                    ...
                                                    <button class="fs-4 mx-4 bg-red-700 py-1 px-2 rounded" name="hotelAddressDelete" type="submit">
            ...
            <div class="mt-8 flex justify-center">
                <?php if ($page > 1): ?>
                    <a href="hotelAddress.php?page=<?php echo $page - 1; ?>" class="px-8  py-2 mx-1 text-white rounded bg-cyan-950 transition duration-700 ease-in-out  hover:bg-transparent hover:outline-1 hover:outline hover:outline-black hover:text-black">Previous</a>
                <?php endif; ?>

                <?php for ($p = 1; $p <= $total_pages; $p++): ?>
                    <a href="hotelAddress.php?page=<?php echo $p; ?>" class="px-8  py-2 mx-1 border border-<?php if ($p == $page){echo 'white';}else{echo 'black';}  ?> rounded <?php if ($p == $page) echo 'bg-cyan-950  text-white'; ?>">
                        <?php echo $p; ?>
                    </a>
                <?php endfor; ?>

                <?php if ($page < $total_pages): ?>
                    <a href="hotelAddress.php?page=<?php echo $page + 1; ?>" class="px-8  py-2 mx-1 text-white rounded bg-cyan-950 transition duration-700 ease-in-out  hover:bg-transparent hover:outline-1 hover:outline hover:outline-black hover:text-black">Next</a>
                <?php endif; ?>
            </div>
        </div>
    </div>
